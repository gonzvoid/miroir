...
let dataFile = path.join(dataDir, 'lumen-data.json');
...
async function loadMetaConfig() {
  try {
    ...
    if (cfg.dataDir) {
      dataDir  = cfg.dataDir;
      dataFile = path.join(dataDir, 'lumen-data.json');
    }
...
// Let the user pick a new folder to store lumen-data.json
ipcMain.handle('data:pickStorageDir', async () => {
  ...
  const newFile = path.join(newDir, 'lumen-data.json');
