...
function BackupSection({ s }) {
  ...
  const exportData = async () => {
    ...
    const name = `lumen-backup-${ymd(new Date())}.json`;
  ...
  const importData = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!Array.isArray(data.tasks)) throw new Error('not a Lumen backup');
  ...
  return (
    <div>
      ...
      <Row label="Restore backup" hint="Replace all data with a previously exported Lumen backup file.">
      ...
      <Row label="Storage location" hint="Where lumen-data.json is saved on this machine.">
